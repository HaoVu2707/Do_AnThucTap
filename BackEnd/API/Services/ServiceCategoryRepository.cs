...
{
    public class ServiceCategoryRepository : GenericRepository<ServiceCatagoryEntity, ServiceCatagoryDto, ServiceCatagoryForCreationDto>, IServiceCategoryRepository
    {
        private DatabaseContext _context;
        private DbSet<ServiceCatagoryEntity> _entity;

        public ServiceCategoryRepository(DatabaseContext context) : base(context)
        {
            _context = context;
            _entity = _context.Set<ServiceCatagoryEntity>();
        }
        new public async Task<Guid> CreateAsync(ServiceCatagoryForCreationDto creationDto)
        {
            var newCompany = new ServiceCatagoryEntity();
        ...
        new public async Task<Guid> EditAsync(Guid id, ServiceCatagoryForCreationDto productDto)
        {
